...
add_action( 'pre_get_posts', 'my_timeline_func' );

function my_timeline_func( WP_Query $query ) {

   if ( $query->is_main_query() && $query->is_tax( 'et-timeline-category' ) ) {
       $query->set( 'post_type', [ 'et-custom-timeline' ] );
   }
}
...
$tags_args = array(
    ...
    'query_var' => true,
    'rewrite' => array( 'slug' => 'et-timeline-tags' ),
);

register_taxonomy( 'et-timeline-tags', 'et-custom-timeline', $tags_args );
add_action( 'pre_get_posts', 'my_timeline_tags' );

function my_timeline_tags( WP_Query $query ) {

   if ( $query->is_main_query() && $query->is_tax( 'et-timeline-tags' ) ) {
       $query->set( 'post_type', [ 'et-custom-timeline' ] );
   }
}
